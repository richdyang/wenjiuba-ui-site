import {
    Component, Input, Output, EventEmitter, ElementRef, OnInit, NgModule, forwardRef,
    Provider
} from '@angular/core';
import { DomSanitizer, SafeHtml } from '@angular/platform-browser';
import { SelectItem } from './select-item';
import {stripTags, HighlightPipe} from './select-pipes';
  ...
  <div tabindex="0"
    ...
    <div class="ui-select-match"
      ...
      <span tabindex="-1"
        ...
        <a *ngIf="allowClear && active.length>0" class="btn btn-link pull-right" style="margin-right: 10px; padding: 0;" (click)="remove(activeOption)">
...
export class SelectComponent implements OnInit, ControlValueAccessor {
  ...
  */
  ...
  writeValue(value:any){
    this._active = this.itemObjects.filter((item:SelectItem) => item.id === value);
  }
  ...
  public doEvent(type:string, value:any):void {
    ...
    if (type == 'selected' || type == 'removed') {
        let value = !this.active || this.active.length == 0 ? null : this.active[0].id
      this.onChange(value);
